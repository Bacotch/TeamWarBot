...
export class UsersDB {
  ...
  constructor(private dbService: Database) {
    ...
    this.verifyUserStmt = this.dbService.prepare(
      "UPDATE users SET discord_user_id = ? , verification_status = 'verified' , verification_code = null WHERE verification_code = ?",
    )
    this.getTeamIdStmt = this.dbService.prepare(
      "SELECT team_id FROM users WHERE verification_status = 'verified' AND discord_user_id = ? ",
    )
  }
